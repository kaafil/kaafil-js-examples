...
**As of:** 2026-08-15 · **re-verified against the newly vendored 214-operation spec** after phases 12
and 13 closed (was written against the 162-operation spec, before either shipped)
...
- ~~**traveller share fetch + forms write-back**~~ — **BOTH SHIPPED (Phase 12).** The line below used to
  read "`shareAuth` … accepted by zero of its 162 operations". The spec is now **214 operations** and
  `shareAuth` is accepted by real ones. These move off `notbuilt` and onto live screens — but note the
  playground cannot exercise them until `kaafil-js` publishes a beta carrying the new resource groups
  (`share`, `forms`, `comms`, `bookings`, `feedback-nps`): the pinned `0.1.0-beta.1` predates them.
