...
import { currentSession, mintSession, sdkCall, sdkClient } from '../live/transport';
import { okFromSdk, okLive, sectionsForSubject, toFail } from '../live/lane';

export const sessionSpecs = (c: any) => ({
  'session.mint': {
    lane: 'B',
    note: 'The one call that crosses the lanes. Your server holds kf_test_… and hands the browser a pair that lives minutes — never the key itself.',
    // Only `managerRef` reaches the engine. `tripRef` and `ttlSeconds` are
    // inputs to YOUR OWN `/session` route — which is why they are labelled as
    // such rather than sitting here looking like part of the wire contract.
    p: [{ n: 'tripRef', l: 'tripRef (your route)', k: 'sel' }, { n: 'managerRef', l: 'managerRef', k: 'text', v: 'mgr_lead_01' }, { n: 'ttlSeconds', l: 'ttlSeconds (your route)', k: 'num', v: 900 }],
    ...
    run: (p: any) => {
      if (!c.sim.trips[p.tripRef]) return c.fail('KaafilNotFoundError', 'RESOURCE_NOT_FOUND', 404, 'No trip resolves for this ref on this tenant.');
      const a = 'kf_mgr_' + Math.random().toString(36).slice(2, 12);
      const r = 'kf_ref_' + Math.random().toString(36).slice(2, 12);
      c.sim.session = { accessToken: a, refreshToken: r, managerRef: p.managerRef, tripRef: p.tripRef, open: false, rotations: 0, expiresIn: Number(p.ttlSeconds) };
      return c.ok({ accessToken: a, refreshToken: r, expiresIn: Number(p.ttlSeconds), managerRef: p.managerRef, role: 'LEAD' });
    },
    live: async (p: any) => {
      try {
        const session = await mintSession({ tripRef: p.tripRef, managerRef: p.managerRef, ttlSeconds: Number(p.ttlSeconds) });
  ...
  'session.open': {
    ...
    live: async () => {
      try {
        ...
        return okLive({ open: true, managerRef: s.managerRef, tripRef: s.tripRef }, null);
      } catch (err) {
        return toFail(err);
      }
    }
  ...
  'session.rotate': {
    lane: 'D',
    note: 'The SDK exchanges the refresh token itself — pre-emptively and on a 401. This button only forces what it already does.',
    p: [],
    ...
    live: async () => {
      try {
        ...
        const body = await client.journey.get({ tripRef: s.tripRef });
  ...
  'session.probe': {
    ...
    live: async () => {
      try {
        ...
        probe.close();
        await probe.journey.get({ tripRef: s.tripRef });
